...
async function callPhotoroomWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!response.ok && isRetryableStatus && attempt < maxRetries) {
        console.log(
          `Photoroom call failed (status ${response.status}), attempt ${attempt}/${maxRetries}. Retrying...`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, retryDelayMs * attempt)
        );
...
// כולל שימור מדויק של המוצר עצמו (כולל פרטים כמו פרצוף).
  ...
  try {
    ...
    const backgroundPrompt = hasUserPrompt
      ...
      : `the image displays ${
        ...
        } placed on a warm wooden surface, with soft natural daylight coming from a window, a softly blurred cozy home interior in the background`;
    ...
    const photoroomResponse =
      await callPhotoroomWithRetry(form);
    ...
    if (!photoroomResponse.ok) {
      ...
      return res
        .status(500)
        .json({ error: "לא הצלחנו לשנות את הרקע" });
    }

    const resultArrayBuffer =
      await photoroomResponse.arrayBuffer();

    const resultBuffer = Buffer.from(resultArrayBuffer);

    res.json({
      image: `data:image/png;base64,${resultBuffer.toString(
        "base64"
      )}`,
    });
