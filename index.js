...
// Does NOT delete processedWebhookEvents (kept for accounting /
// payment-dispute purposes - disclosed in Privacy Policy).
// Does NOT cancel any active Google Play subscription - that must
// be cancelled separately by the user via Google Play.
// ============================================================
 
async function deleteCollectionInBatches(collectionRef, batchSize = 450) {
  ...
  // If we deleted a full batch, there may be more - recurse.
...
app.post("/delete-account", requireAuth, async (req, res) => {
  try {
    ...
    // 1. Delete postImproveCounts subcollection, in safe batches
    const improveCountsRef = db
      .collection("users")
      .doc(uid)
      .collection("postImproveCounts");
 
    await deleteCollectionInBatches(improveCountsRef);
 
    // 2. Delete the main user document
    await db.collection("users").doc(uid).delete();
 
    // 3. Delete the Firebase Auth account (must be last - after this,
