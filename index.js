...
const PLAN_BY_PRODUCT_ID = {
  "postly_starter_monthly:starter-monthly": { plan: "starter", credits: 10 },
  "postly_pro_monthly:pro-monthly": { plan: "pro", credits: 55 },
  "postly_business_monthly:business-monthly": { plan: "business", credits: 100 },
 
  "postly_starter_monthly": { plan: "starter", credits: 10 },
  "postly_pro_monthly": { plan: "pro", credits: 55 },
  "postly_business_monthly": { plan: "business", credits: 100 },
};
...
// ============================================================
// PLANS (public, no auth needed - just plan metadata for the Paywall)
// ============================================================
//
// Lets the app show current credit amounts without them being hardcoded
// in the Frontend - change PLAN_BY_PRODUCT_ID above and redeploy, and the
// app picks it up on next load, no new Android/iOS version needed.
// ============================================================
 
const PLAN_TITLES_HE = {
  starter: "Starter",
  pro: "Pro",
  business: "Business",
};
...
app.get("/plans", (req, res) => {
  // Derive one entry per plan (starter/pro/business) from
  // PLAN_BY_PRODUCT_ID, which is already the single source of truth for
  // credit amounts (used by the webhook to actually grant credits).
  const plans = {};
  for (const info of Object.values(PLAN_BY_PRODUCT_ID)) {
    plans[info.plan] = {
      title: PLAN_TITLES_HE[info.plan] || info.plan,
