...
// יוצר תמונת רקע חדשה מאפס (בלי שום מוצר בתוכה) לפי תיאור - עם ניסיונות חוזרים
...
// endpoint: שינוי הרקע של תמונת המוצר, בשיטת "הפרדה מלאה":
// 1. remove.bg מפריד את המוצר מהרקע המקורי (PNG שקוף, המוצר בלבד)
// 2. OpenAI מייצר רקע חדש לגמרי מאפס - בלי שום מוצר בתוכו, בדיוק לפי הבקשה
// 3. sharp מדביק צל מבוקר (שאנחנו יוצרים, לא ה-AI) ואת המוצר המדויק על הרקע החדש
// כך יש לנו שליטה מלאה על הרקע (בדיוק לפי הבקשה, בלי טשטוש כפוי), והמוצר נשמר פיקסל-מדויק.
app.post("/change-background", upload.single("image"), async (req, res) => {
  try {
    ...
    // שלב 2: יצירת רקע חדש, בלי שום מוצר בתוכו - בדיוק לפי הבקשה, בלי טשטוש כפוי
    const backgroundGenPrompt = hasUserPrompt
      ? `A high-quality photograph of an empty scene: ${userPrompt.trim()}. The scene is completely empty - no products, no objects, no people, no animals. Sharp focus throughout, no blur, no depth-of-field effect. This is a background plate ready for a subject to be placed on top of it later.`
      : `A high-quality photograph of an empty, clean, professional background scene suitable for product photography of: ${description || "a product"}. The scene is completely empty - no products, no objects, no people, no animals. Sharp focus throughout, no blur.`;
    ...
    // שלב 3: הכנת המוצר בגודל סופי, ובניית צל מבוקר מתחתיו
    const canvasSize = 1024;
    const targetWidth = Math.round(canvasSize * 0.62);
    ...
    const productWidth = resizedMeta.width;
    const productHeight = resizedMeta.height;
 
    // מיקום המוצר: ממורכז אופקית, מעט מתחת למרכז אנכית (כדי שיהיה מקום לצל "לשבת" מתחתיו)
    const productLeft = Math.round((canvasSize - productWidth) / 2);
    const productTop = Math.round((canvasSize - productHeight) / 2 + productHeight * 0.12);
 
    // צל אליפסה מטושטש, ממוקם בדיוק מתחת לבסיס המוצר
    const shadowWidth = Math.round(productWidth * 0.85);
    const shadowHeight = Math.round(shadowWidth * 0.22);
    const shadowSvg = `<svg width="${shadowWidth}" height="${shadowHeight}" xmlns="http://www.w3.org/2000/svg">
      <ellipse cx="${shadowWidth / 2}" cy="${shadowHeight / 2}" rx="${shadowWidth / 2}" ry="${shadowHeight / 2}" fill="black" fill-opacity="0.32"/>
    </svg>`;
 
    const shadowBuffer = await sharp(Buffer.from(shadowSvg))
      .blur(Math.round(shadowHeight * 0.4) || 1)
      .toBuffer();
 
    const shadowLeft = Math.round((canvasSize - shadowWidth) / 2);
    const shadowTop = Math.round(productTop + productHeight - shadowHeight * 0.55);
    ...
    const finalBuffer = await sharp(backgroundBuffer)
      ...
      .composite([
        ...
        { input: resizedCutout, left: productLeft, top: productTop },
      ])
      .png()
      .toBuffer();
