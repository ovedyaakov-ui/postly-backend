  ...
  try {
    ...
    if (hasFace) {
      ...
      const backgroundGenPrompt = hasUserPrompt
        ...
        : `תמונת רקע ריקה (בלי אף מוצר, בלי אף חפץ מרכזי, בלי בני אדם או בעלי חיים) שמתאימה באופן טבעי והגיוני לתמונה הבאה: ${description || "תמונה כללית"}. ${contrastInstruction}
בחר סגנון רקע שמתאים להקשר האמיתי - למשל: אם זו חיית מחמד, רקע חם וביתי (כמו רצפת עץ, שטיח, גינה, ספה) ולא רקע סטודיו קר; אם זה מוצר צריכה, רקע נקי ומקצועי; אם זה מזון, משטח שמתאים להגשה. הרקע לא צריך להיות "ריק" מדי או משעמם - הוא צריך להרגיש כמו סביבה אמיתית והגיונית, לא רק צבע אחיד.`;
